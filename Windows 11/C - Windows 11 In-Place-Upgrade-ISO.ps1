...
Write-Output "Suspending BitLocker at $(Get-Date)" | Out-File "$dir\Upgrade.log" -Append
...
foreach ($vol in $bitlockerVolumes) {
    ...
    Write-Output "BitLocker suspended on $($vol.MountPoint)" | Out-File "$dir\Upgrade.log" -Append
}
...
foreach ($vol in $bitlockerVolumes) {
    ...
    Write-Output "BitLocker resumed on $($vol.MountPoint) at $(Get-Date)" | Out-File "$dir\Upgrade.log" -Append
}
